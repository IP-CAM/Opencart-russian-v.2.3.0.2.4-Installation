		...
		if (isset($this->request->post['banner_image'])) {
			foreach ($this->request->post['banner_image'] as $language_id => $value) {
				foreach ($value as $banner_image_id => $banner_image) {
					...
					if (strtotime($banner_image['date_start']) > strtotime($banner_image['date_end'])){
						$this->error['banner_image'][$language_id]['date' . $banner_image_id] = $this->language->get('error_date');
					}
